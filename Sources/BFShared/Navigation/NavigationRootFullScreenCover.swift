...
#if !os(macOS)
public struct NavigationRootFullScreenCover<Root>: View where Root: View {
...
public extension View {
    func fullScreenCover<Root: View>(_ path: Navigation.FullScreenCoverPath,
                                     _ root: @escaping () -> Root,
                                     onDismiss: (() -> Void)? = nil) -> some View {
        modifier(NavigationRootFullScreenCoverModifier(path: path, root, onDismiss: onDismiss))
    }
}

public struct NavigationRootFullScreenCoverModifier<Root: View>: ViewModifier {
    ...
    public func body(content: Content) -> some View {
        content
            .background(
                NavigationRootFullScreenCover(path: path, root, onDismiss: onDismiss)
            )
    }
}
#endif
